...
export type EnlaceAsistente = 'inventario' | 'nueva_planta' | 'nuevo_tratamiento' | 'recorrido' | 'ajustes';
  ...
  try {
    const { object } = await generateObject({
      ...
      schema: z.object({
        ...
        enlaces: z.array(z.enum(['inventario', 'nueva_planta', 'nuevo_tratamiento', 'recorrido', 'ajustes'])).describe('Pantallas que conviene ofrecerle para completar lo que pide. Vacío si no hace falta ninguna.'),
      }),
...
3. Si ha comprado un producto → dalo de alta con lo que diga y ofrece el enlace "inventario" para completar la ficha o escanear la etiqueta. Si no da ni el nombre, solo el enlace.
...
5. Si pide consejo o recomendación → contesta tú directamente, concreto y con los datos del jardín (qué producto del inventario usar, cuándo). NO registres nada salvo que lo pida.
